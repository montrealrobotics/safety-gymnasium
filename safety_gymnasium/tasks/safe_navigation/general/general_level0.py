...
class GeneralLevel0(BaseTask):
    ...
    def __init__(self, config) -> None:
        ...
        self._add_geoms(Buttons(num=1, is_constrained=True))
        self._add_geoms(Goal(size=self.buttons.size * 2, alpha=0.1))  # pylint: disable=no-member
        self._add_free_geoms(PushBox(null_dist=0), Vases(num=1, is_constrained=False))
        self._add_mocaps(Gremlins(num=1, travel=0.35, keepout=0.4))
        self._add_geoms(Hazards(num=1, size=0.3), Pillars(num=1, is_constrained=True))
